...
const ActiveJobsWidget = () => {
  const [activeJobs, setActiveJobs] = useState(ACTIVE_JOBS);
    ...
    const fetchJobs = async () => {
      try {
        ...
        const data = (res.data.jobs || [])
          ...
          .slice(0, 4) // Limit to 4 for widget
          .map((j, idx) => ({
            id: j._id,
            title: j.title,
            applicants: j.applicantCount || 0,
            newToday: Math.floor(Math.random() * 5), // Mock new today
            daysLeft: 20 - idx * 2, // Mock days left
            progress: 30 + idx * 15,
          }));
        setActiveJobs(data.length > 0 ? data : ACTIVE_JOBS);
      } catch (error) {
        console.error('Failed to fetch jobs:', error);
        setActiveJobs(ACTIVE_JOBS);
      } finally {
        setLoading(false);
      }
    };
