    ...
    (async () => {
      try {
        ...
        const data = (res.data?.interviews || []).map(iv => ({
          id:        iv._id,
          candidate: `${iv.candidateId?.firstName || ''} ${iv.candidateId?.lastName || ''}`.trim() || 'Unknown',
          role:      iv.jobId?.title || 'Position',
          time:      iv.interviewDate
            ? new Date(iv.interviewDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) + (iv.interviewTime ? `, ${iv.interviewTime}` : '')
            : '—',
    ...
    <div className="bg-white rounded-2xl border border-neutral-100 p-5">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-neutral-400 mt-0.5">Scheduled interviews</p>
        </div>
      ...
      {loading ? (
        ...
        <p className="text-xs text-neutral-400 text-center py-6">No upcoming interviews scheduled.</p>
      ) : (
