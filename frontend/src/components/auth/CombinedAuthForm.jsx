  ...
  return (
    <div className="relative pt-2">
      <div className={`relative group transition-all duration-300 ${isFocused ? 'scale-[1.01]' : ''}`}>
        <div className={`absolute left-3.5 top-1/2 -translate-y-1/2 transition-colors duration-200 z-10 ${isFocused ? 'text-brand-purple-500' : 'text-neutral-400'}`}>
          <Icon size={18} />
        </div>
        ...
        <input
          ...
          className={`peer w-full pl-11 pr-11 py-3.5 bg-neutral-50 border-2 rounded-2xl text-sm transition-all duration-300 outline-none
            ${error ? 'border-red-200 focus:border-red-500 bg-red-50/30' : 'border-neutral-100 focus:border-brand-purple-500 focus:bg-white focus:shadow-md'}
          `}
          placeholder=" "
        />
        ...
        {/* Floating Label */}
        <label className={`
          absolute left-11 top-1/2 -translate-y-1/2 text-sm transition-all duration-300 pointer-events-none
          peer-placeholder-shown:text-neutral-400
          ${(hasValue || isFocused) 
            ? '-top-2 left-4 text-xs font-bold text-brand-purple-600 bg-white px-2' 
            : ''}
        `}>
          {label}
        </label>
        ...
        {showPasswordToggle && (
          <button
            ...
            className="absolute right-3.5 top-1/2 -translate-y-1/2 text-neutral-400 hover:text-brand-purple-600 transition-colors z-10"
          >
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            className="flex items-center gap-1 mt-1.5 ml-1 text-red-500"
          >
            <div className="w-1 h-1 rounded-full bg-red-500" />
            <p className="text-[11px] font-semibold tracking-tight">{error.message}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
